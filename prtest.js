(function () {
    ...
    // 1. То самое уведомление от Лампы, которое успешно работало в 5-й версии!
    ...
    setTimeout(function () {
        showNoty('[Torrents Copy v' + VERSION + '] Плагин успешно загружен!');
    }, 1500);

    // 2. Перехват шаблонов (сохраняем данные раздачи в элемент, как делают цветные плагины)
    ...
    // 3. Запоминаем последний элемент, которого коснулись пальцем, мышкой или пультом
    ...
    // 4. Универсальный поиск ссылок во всех полях раздачи
    ...
    function hookSelect() {
        ...
        Lampa.Select.show = function (params) {
            if (params && params.items && Array.isArray(params.items)) {
                ...
                // 2. Ищем в элементе, которого коснулись пальцем или пультом
                ...
                // 3. Ищем в текущем фокусе на экране
                ...
                // Если ссылка найдена — добавляем пункты!
