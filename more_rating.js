(function () {
    ...
    // 1. ВНЕДРЯЕМ БРОНЕБОЙНЫЕ СТИЛИ (с максимальным z-index: 9999)
    let style = document.createElement('style');
    style.innerHTML = `
        /* --- Общие настройки: рисуем ПОВЕРХ ВСЕГО --- */
        .my-overlay-box {
    ...
    // 2. УНИВЕРСАЛЬНЫЙ ПОИСК И ПРИКЛЕИВАНИЕ
    Lampa.Listener.follow('card', function (e) {
        if (e.type == 'build') {
            try {
                ...
                // Агрессивный поиск: ищем .card__view, если нет — .card__img, если нет — цепляемся за саму карточку!
                let view = null;
                if (card.find) {
                ...
                // Принудительно удерживаем границы, чтобы плашки не улетали за постер
                ...
                console.log('Ошибка отрисовки оверлея:', err);
            }
        }
    });

    console.log('Бронебойный плагин оверлея успешно загружен!');
